...
export class AstrologySourceError extends Error {
  constructor(message = "目前沒有足夠的可追溯占星來源完成這次解讀。") {
    super(message);
    this.name = "AstrologySourceError";
    this.code = "ASTROLOGY_SOURCES_NOT_READY";
  }
}
...
export function normalizeAstrologyRequest(input) {
  ...
  if (!sign) throw new AstrologyValidationError("請先輸入生日。", "INVALID_SIGN");
...
export function normalizeAstrologyOutput(value) {
  ...
  if (!Array.isArray(value.focusAreas) || value.focusAreas.length < 1 || value.focusAreas.length > 2) {
    throw new AstrologyOutputError("星座解讀缺少額外值得留意的生活角度。");
  }
  const focusAreas = value.focusAreas.map((item) => ({
    title: String(item?.title || "").trim(),
    body: String(item?.body || "").trim(),
  }));
  if (focusAreas.some((item) => !item.title || !item.body)) {
    throw new AstrologyOutputError("星座解讀的額外生活角度格式不完整。");
  }
...
function astrologySpeechFallback(result) {
  return {
    overview: result.overview,
    narrative: result.overall,
  };
}
...
export async function createAstrologyReading(input, { openai = null, model = CHAT_MODEL } = {}) {
  ...
  if (!evidence.sufficient) throw new AstrologySourceError();

  const client = openai || getOpenAI();
  const response = await client.responses.create({
    model,
    instructions: buildAstrologyInstructions(),
    input: buildAstrologyInput({ ...normalized, skyContext, evidence }),
    text: {
      format: {
        type: "json_schema",
  ...
  const result = normalizeAstrologyOutput(parsed);
  const sign = publicZodiacSign(normalized.sign);
  const renderedSpeech = await renderVelaSpeech({
    client,
    model,
    mode: "astrology",
    input: buildAstrologySpeechInput({
      sign,
      period: normalized.period,
      localDate: normalized.localDate,
      analysis: result,
    }),
    fallback: astrologySpeechFallback(result),
  });
  ...
  return {
    ...
    sourceGrounded: true,
    disclaimer: "星座解讀是以歷史占星來源與當天近似太陽／月亮位置做的象徵性反思與娛樂性參考，不保證事件發生，也不取代醫療、法律、財務或其他專業判斷。",
  };
}
