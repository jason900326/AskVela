...
function normalizeClarifierCount(value) {
  ...
  if (!Number.isInteger(count) || count < 0 || count >= TAROT_CLARIFIER_LIMIT) {
    throw new ClarifierValidationError(
      `同一次占卜最多補 ${TAROT_CLARIFIER_LIMIT} 張牌。`,
      "CLARIFIER_LIMIT_REACHED",
    );
  }
  return count;
}
...
function validatePayload(body = {}) {
  return {
    ...
    clarifierCount: normalizeClarifierCount(body.clarifierCount),
    initialReading: normalizeInitialReading(body.initialReading),
  };
}

function chooseClarifier({ fixedDraw, existingCount, secret }) {
  const used = new Set(fixedDraw.cards.map((card) => card.cardId));
  let chosen = null;

  for (let ordinal = 1; ordinal <= existingCount + 1; ordinal += 1) {
    const candidates = TAROT_CARDS.filter((card) => !used.has(card.card_id));
    const cardDigest = createHmac("sha256", secret)
      .update(`clarifier:${fixedDraw.readingId}:${ordinal}`)
      .digest();
    const orientationDigest = createHmac("sha256", secret)
      .update(`clarifier-orientation:${fixedDraw.readingId}:${ordinal}`)
      .digest();
    const card = candidates[cardDigest.readUInt32BE(0) % candidates.length];
    const orientation = orientationDigest.readUInt32BE(0) % 2 === 0 ? "upright" : "reversed";
    used.add(card.card_id);
    chosen = { card, orientation, ordinal };
  }
...
export async function drawReadingClarifier(
  ...
  const chosen = chooseClarifier({ fixedDraw, existingCount: input.clarifierCount, secret });
  ...
  return {
    ...
    disclaimer: "這是一張補充牌，用來釐清原本牌面的某個角度，不會取代或重算前面的牌陣。",
  };
}
