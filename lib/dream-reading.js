...
export function normalizeDreamRequest(input) {
  ...
  if (dreamText.length < 8) throw new DreamValidationError("請再多描述一點夢裡發生的事情。", "DREAM_TOO_SHORT");
...
export function normalizeDreamOutput(value, allowedEvidenceIds = []) {
  ...
  if (whatStandsOut.length < 2 || reflectionQuestions.length < 2) throw new DreamOutputError();
...
export async function createDreamReading(input, { openai = null, model = CHAT_MODEL } = {}) {
  ...
  return {
    ...
    sourceGrounded: true,
    disclaimer: "解夢內容是以歷史心理學文本作為象徵性反思框架，不是診斷、治療、預言或固定夢境字典。夢的意義仍以你自己的聯想、生活脈絡與感受為主。",
  };
}
