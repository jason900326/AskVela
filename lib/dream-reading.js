...
function dreamSpeechFallback(result) {
  return {
    overview: result.overview,
    narrative: result.hypotheses?.[0]?.interpretation || result.wakingLifeConnection,
  };
}

export async function createDreamReading(
  input,
  {
    openai = null,
    model = CHAT_MODEL,
    retrievePassages = retrieveFreudDreamPassages,
  } = {},
) {
  ...
  try {
    retrievedPassages = await retrievePassages({
      dreamText: normalized.dreamText,
      wakingLifeContext: normalized.wakingLifeContext,
      extraction,
    });
  } catch {
    // Retrieval failure must not erase the user's Dream session. The curated
    // Freud corpus remains a traceable fallback until the index is available.
    retrievedPassages = [];
  }

  const hasRetrievedPassages = retrievedPassages.length > 0;
  const readingResponse = await client.responses.create({
    model,
    instructions: buildDreamInstructions({ hasRetrievedPassages }),
    input: buildDreamInput({
      ...normalized,
      extraction,
      evidence,
      retrievedPassages,
  ...
  const safety = classifyHighStakesQuestion(`${normalized.dreamText}\n${normalized.wakingLifeContext}`);
  const renderedSpeech = await renderVelaSpeech({
    client,
    model,
    mode: "dream",
    input: buildDreamSpeechInput({
      dreamText: normalized.dreamText,
      wakingLifeContext: normalized.wakingLifeContext,
      analysis: result,
    }),
    fallback: dreamSpeechFallback(result),
    safety,
  });
  const { speech, status, attempts } = renderedSpeech;
  ...
  return {
    ...
    retrievedSources: publicFreudDreamPassages(retrievedPassages),
    sourceGrounded: true,
    sourceMode: hasRetrievedPassages ? DREAM_SOURCE_METHOD.fullRagMode : DREAM_SOURCE_METHOD.fallbackMode,
    sourceNote: hasRetrievedPassages ? DREAM_SOURCE_METHOD.fullRagNote : DREAM_SOURCE_METHOD.fallbackNote,
