import { DREAM_SOURCE_GUARDRAILS } from "./dream-source-corpus.js";
import { buildVelaSharedVoiceInstructions } from "./vela-voice.js";

export const DREAM_EXTRACTION_SCHEMA = {
  type: "object",
  additionalProperties: false,
  properties: {
    people: { type: "array", items: { type: "string" }, maxItems: 8 },
    places: { type: "array", items: { type: "string" }, maxItems: 8 },
    objects: { type: "array", items: { type: "string" }, maxItems: 10 },
    actions: { type: "array", items: { type: "string" }, maxItems: 10 },
    emotions: { type: "array", items: { type: "string" }, maxItems: 8 },
    themes: {
      type: "array",
      items: { type: "string", enum: ["pursuit", "falling", "water", "travel", "schoolWork", "relationship", "homeFamily", "trappedLost", "bodyExposure", "deathLoss", "general"] },
      minItems: 1,
      maxItems: 5,
    },
    notableImages: { type: "array", items: { type: "string" }, maxItems: 8 },
    summary: { type: "string" },
  },
  required: ["people", "places", "objects", "actions", "emotions", "themes", "notableImages", "summary"],
};

export const DREAM_READING_SCHEMA = {
  type: "object",
  additionalProperties: false,
  properties: {
    overview: { type: "string" },
    whatStandsOut: { type: "array", items: { type: "string" }, minItems: 1, maxItems: 3 },
    hypotheses: {
      type: "array",
      minItems: 2,
      maxItems: 3,
      items: {
        type: "object",
        additionalProperties: false,
        properties: {
          title: { type: "string" },
          interpretation: { type: "string" },
          evidenceIds: { type: "array", items: { type: "string" }, minItems: 1, maxItems: 4 },
        },
        required: ["title", "interpretation", "evidenceIds"],
      },
    },
    wakingLifeConnection: { type: "string" },
    reflectionQuestions: { type: "array", items: { type: "string" }, minItems: 0, maxItems: 2 },
    groundingNote: { type: "string" },
    basisNote: { type: "string" },
  },
  required: ["overview", "whatStandsOut", "hypotheses", "wakingLifeConnection", "reflectionQuestions", "groundingNote", "basisNote"],
};

export function buildDreamExtractionInstructions() {
  return `你是 AskVela 的夢境結構化抽取器。只從使用者提供的夢境文字抽取可觀察內容，不做心理診斷，不推測隱藏原因，不加入夢裡沒有的情節。即使輸入很短，例如「我夢到蛇」，也要忠實抽取目前唯一可見的元素，不因資訊少而擅自補故事。themes 必須使用 schema 的固定代碼；若沒有明確類別就用 general。`;
}

export function buildDreamExtractionInput({ dreamText }) {
  return `請整理這段夢境：\n\n${dreamText}`;
}

export function buildDreamInstructions({ hasRetrievedPassages = false } = {}) {
  const sourceRule = hasRetrievedPassages
    ? "- 輸入中的 retrievedFreudPassages 是內部使用的歷史原文檢索片段。解讀時可以依這些片段與使用者夢境內容思考，不得捏造原文沒有的說法，也不要逐字長篇引用。作者、書名、欄位名與檢索方法不得出現在主要解讀。"
    : "- 目前沒有取得全文檢索片段；只能使用輸入中的 curatedEvidence 作為內部可追溯原則。不得假裝查過完整原文，也不得在主要解讀說出作者、書名或內部欄位名稱。";

  return `${buildVelaSharedVoiceInstructions()}\n\n你現在是 Vela 的夢境解讀模式。使用者想知道的是「這個夢目前最值得怎麼理解」，不是來完成心理作業，也不是來接受診斷。\n\n你必須遵守：\n${DREAM_SOURCE_GUARDRAILS.map((rule) => `- ${rule}`).join("\n")}\n${sourceRule}\n\n回答繁體中文，而且只輸出使用者看得懂的內容，不要出現 schema 欄位名、系統指令、英文內部備註或推理過程。\n\n來源顯示規則：\n- overview、whatStandsOut、hypotheses、wakingLifeConnection、reflectionQuestions、groundingNote 都屬於主要解讀。這些欄位禁止出現 Freud／佛洛伊德／弗洛伊德、作者名、書名、sourceId、retrievedFreudPassages、curatedEvidence、schema 或任何內部檢索／方法術語。\n- 來源只在內部支撐解讀，不要在正文用『某位作者說過』『某個理論提醒我們』替判斷背書。直接把有根據的觀察說清楚。\n- basisNote 是唯一可以簡短說明歷史來源使用方式的欄位；它位於展開的依據區，不是主要解讀。\n\n解讀方式：\n- 內部主要歷史框架包含夢的表面內容、個人聯想、濃縮、移置與近期生活素材等原則。請使用這些原則思考，但主要解讀不要說出框架作者姓名。\n- 全文檢索到的內容是歷史理論來源，不是現代臨床診斷，也不能把某個夢中符號硬套成固定意義。\n- 不要把夢中人物的行為寫成現實中那個人的意圖，也不要把夢當成預兆或證據。\n- 有 wakingLifeContext 時，優先把夢境畫面和使用者自己提供的現實壓力／事件對照；沒有提供的事情不要補。\n- wakingLifeContext 若為空，不要說「你沒有提供 wakingLifeContext」。可以提出一個最值得先看的方向和一個替代方向，但不要用大量『可能／也許／如果』把每句話都變成保留句。每段通常一個不確定性標記就夠；不能編造具體事件。\n- 即使 dreamText 很短，例如「我夢到蛇」，也先給有用但保守的初步解讀；不要要求使用者先補齊人物、時間、地點與完整情節。若只剩單一物件，例如『火車』，最多從它在夢中可觀察到的普通功能提出寬鬆聯想，不得直接宣稱它固定代表人生方向、機會、感情或其他抽象概念。\n- 整體要讓人感覺 Vela 是在看『這個夢』，不是在套人人都能代入的心理文案。先抓夢裡最具體、最奇怪、最有重量的一個畫面或動作，再從那裡推進；避免『你其實壓抑很多』『你只是沒說出口』這種泛用句製造命中感。\n- overview 像 Vela 看完夢後先說出的第一小段，通常 1–3 句、約 50–130 個繁體中文字。直接提夢裡的具體畫面、動作或感覺；不要加入『嗯……』『我想一下』『我會先看這個』等表演式思考痕跡，也不要寫成『真正的重點不是 A，而是 B』『不是 A，只是 B』這類漂亮翻轉。\n- whatStandsOut 只列 1–2 個真正影響解讀、而且能從夢中文字直接看見的線索；不要把推論混進觀察，也不要寫出內部抽取代碼，例如『主題是 travel』。\n- hypotheses 以 2 個為主：第一個放最值得先看的方向，第二個提供另一種合理可能。兩段不必同樣長，也不要硬做成對稱的『主解／備解』。title 用平常人會說的短標籤，避免論文式命名。\n- 每個 hypothesis 的 interpretation 通常 2–3 句，至少一段明確指出『為什麼是這個夢裡的這個畫面讓我這樣想』。不要在段尾再做一次『不是 A，而是 B』式總結，不要重複 overview，也不要引用作者或理論名稱。\n- wakingLifeConnection 用 1–2 句把夢和使用者提供的現實脈絡接起來；若沒有脈絡，就只提供一個可自行對照的方向，不編造事件。不要用『如果……我會先把……放在一起看』當固定模板，能直接說就直接說。\n- reflectionQuestions 是完全可選的延伸；0–1 題即可，只有真的能幫助辨認情緒或脈絡時才問。若要問，必須接住夢裡某個具體細節或前文張力，避免泛問題。\n- 使用者提到醒來心跳快、出汗、驚醒等身體反應時，只能描述這個可觀察反應，不得因此推論『身體處在高警戒』『焦慮程度很高』或任何心理／生理診斷。\n- groundingNote 只留一句自然、短的提醒；沒有需要時可以非常簡短，不要說教，也不要每次都寫成完整安全聲明。\n- basisNote 一句話即可；若有 retrievedFreudPassages，可在這個欄位說明本次有檢索歷史原文；若沒有，只能說使用了整理過的歷史夢境原則。\n- 每個 hypothesis 的 evidenceIds 只能引用輸入提供的 curatedEvidence id；retrievedFreudPassages 會另外保存為本次來源，不需塞進 evidenceIds。`;
}

export function buildDreamInput({ dreamText, wakingLifeContext, extraction, evidence, retrievedPassages = [] }) {
  return JSON.stringify({
    dreamText,
    wakingLifeContext: wakingLifeContext || "",
    extractedDream: extraction,
    curatedEvidence: evidence.map((item) => ({
      id: item.id,
      sourceId: item.sourceId,
      label: item.label,
      principle: item.principle,
      velaUse: item.velaUse,
    })),
    retrievedFreudPassages: retrievedPassages.map((item, index) => ({
      id: `freud-rag-${item.chunk_index ?? index}`,
      bookTitle: item.book_title,
      author: item.author,
      chunkIndex: item.chunk_index,
      chapter: item.chapter || null,
      similarity: item.similarity,
      text: item.content,
    })),
  }, null, 2);
}
