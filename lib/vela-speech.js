...
export const VELA_SPEECH_VERSION = "shared-speech-v3";
...
const POLISHED_CONTRAST_PATTERN = /不是[^。！？\n]{0,36}(?:而是|反而是|只是)|與其[^。！？\n]{0,36}不如|看似[^。！？\n]{0,36}其實|真正(?:的)?(?:重點|問題|需要|值得)|核心(?:在於|是)|核心不在[^。！？\n]{0,36}而在|關鍵不在[^。！？\n]{0,36}而在/u;
const PERFORMATIVE_HUMAN_PATTERN = /嗯[………\.]{2,}|我想一下|怎麼說[………\.]*|我會先看這個|這裡有個地方我會多看一眼/u;
...
export function buildVelaSpeechInstructions(
  ...
  return [
    ...
    "overview 是 Vela 開口的第一句，不是標題、摘要或金句。可以短，但要完整、清楚，不要為了漂亮而做對稱句或翻轉句。",
    "narrative 通常 3–6 句、約 100–300 個繁體中文字。句子長短可以自然不同，不需要每句都有結論。",
    "禁止使用『不是 A，而是 B』『不是 A，只是 B』『與其 A，不如 B』『看似 A，其實 B』『真正的重點是』『核心在於』『關鍵不在 A 而在 B』等工整翻轉／對偶句。需要校正誤解時，直接說目前能判定什麼、不能判定什麼。",
    "不要刻意模擬人類正在思考。預設不用『嗯……』『我想一下』『怎麼說……』『我會先看這個』『這裡有個地方我會多看一眼』、半句停住、破折號改口或輕微重複來製造人味。",
    "如果內容涉及另一個人的愛不愛、想法、動機或內在狀態，不要把分析裡的可能性改寫成『他現在……』『她其實……』『對方心裡……』這種陳述。",
    "每句盡量帶新資訊。可以在一個觀察上停住，但不要把同一個感覺換句話說多次，也不要每段最後都補『所以你可以……』。",
    ...
    retry
      ? "這是 Speech Layer 的修正重試。上一版沒有通過呈現規則；這次更保守：避開二分翻轉句、表演式思考痕跡、第三人稱內在斷言與高風險行動指令，只輸出乾淨、直接的 overview 與 narrative。"
      : "",
  ].filter(Boolean).join("\n");
}
...
export function buildTarotSpeechInput({ reading, cards, synthesis, safety }) {
  return JSON.stringify({
    task: "把已完成的 grounded Tarot analysis 改寫成簡短、直接的 Vela 口吻。",
...
export function buildAstrologySpeechInput({ sign, period, localDate, analysis }) {
  return JSON.stringify({
    task: "把已完成、來源型的 Astrology analysis 改寫成簡短、直接的 Vela 口吻。",
    readingLens: {
      signId: sign.id,
...
export function buildDreamSpeechInput({ dreamText, wakingLifeContext, analysis }) {
  return JSON.stringify({
    task: "把已完成、來源型的 Dream analysis 改寫成簡短、直接的 Vela 口吻。",
    userDream: dreamText,
    wakingLifeContext,
    groundedAnalysis: analysis,
  }, null, 2);
...
function cleanSpeechText(value, {
  ...
  if (PERFORMATIVE_HUMAN_PATTERN.test(text)) {
    return { text: fallback, usedFallback: true, violation: "performative-humanity" };
  }
...
export function buildHighStakesSpeechFallback(safety = {}) {
  ...
  const selected = copy[category] || {
    ...
    narrative: normalizedSafety.notices[0] || "可以用這次解讀整理感受和選擇壓力；實際決定仍要回到可靠資訊與適合的專業協助。",
  };
