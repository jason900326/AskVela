...
function cleanSpeechText(value, { min, max, fallback }) {
  ...
  if (text.length < min || text.length > max) return fallback;
  if (/^(?:#|[-*]\s|\d+[.)]\s)/mu.test(text)) return fallback;
  return text;
}
...
export function normalizeVelaSpeech(output, fallback) {
  ...
  return {
    overview,
    narrative,
    usedFallback: overview === fallback.overview && narrative === fallback.narrative,
  };
}
