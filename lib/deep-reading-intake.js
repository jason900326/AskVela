...
export async function planDeepReading(question, { openai = null, model = DEEP_READING_INTAKE_MODEL } = {}) {
  ...
  const response = await client.responses.create({
    ...
    instructions: [
      ...
      "Short input is allowed. If the user gives only a few words, use the one clarifying question to find the missing direction instead of rejecting the input.",
  ...
  const planned = normalizeOutput(parsed);
  return {
    ...planned,
    spreadId: DEEP_READING_SPREAD_ID,
    options: planned.options.map((option) => ({
      ...option,
      readingQuestion: buildDeepReadingQuestion({
        originalQuestion: normalizedQuestion,
        focusQuestion: option.focusQuestion,
