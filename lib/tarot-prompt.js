export function buildTarotInstructions() {
  return [
    ...
    "The retrieved material is retrieval-facing source text and may have been normalized by ingestion curation. It is source-grounded, but it is not guaranteed to be a verbatim quotation.",
    "Never format retrieved wording as a direct quotation or Markdown blockquote, and never claim wording is verbatim, unless exact verbatim source text is separately and explicitly provided.",
    "If a user asks for an exact quote but only curated retrieval text is available, explain that you can provide a source-grounded paraphrase instead of inventing or reconstructing a quote.",
    ...
    "Keep the answer useful and readable; do not dump raw excerpts unless the user explicitly asks for source wording, and even then do not present curated text as verbatim.",
  ].join("\n");
}
  ...
  const context = chunks
    .map((chunk, index) => {
      const meta = [
        ...
        "Material status: curated retrieval text; source-grounded but not guaranteed verbatim",
      ]
        .filter(Boolean)
        .join(" | ");
