...
export class FollowUpMismatchError extends Error {
  constructor(message = "這次追問與原本的占卜不一致；請保留同一個 readingId、requestId、問題、牌陣與選牌結果後重試。") {
    super(message);
...
function cleanText(value, maxCharacters, fieldName, { required = false } = {}) {
  // Preserve user-visible punctuation because the exact original question is part of
  // the deterministic draw identity. Compatibility normalization (NFKC) would turn
  // full-width Chinese punctuation into ASCII and silently change the reading.
...
function normalizeSelectedCardIndexes(value) {
  if (value == null) return null;
  if (!Array.isArray(value) || value.length < 1 || value.length > 3) {
    throw new FollowUpValidationError("選牌紀錄格式不正確。", "INVALID_FOLLOW_UP_SELECTION");
  }
  ...
  if (indexes.some((index) => !Number.isInteger(index) || index < 0 || index >= TAROT_SELECTION_POOL_SIZE)) {
    throw new FollowUpValidationError("選牌紀錄超出可用範圍。", "INVALID_FOLLOW_UP_SELECTION");
  }
  if (new Set(indexes).size !== indexes.length) {
    throw new FollowUpValidationError("選牌紀錄包含重複位置。", "INVALID_FOLLOW_UP_SELECTION");
  }
  return indexes;
}
...
export function validateFollowUpPayload(body = {}) {
  ...
  return {
    ...
    question,
    spreadId,
    selectedCardIndexes: normalizeSelectedCardIndexes(body.selectedCardIndexes),
    message,
...
export function buildFollowUpInstructions(safety) {
  return [
    ...
    "If source authors disagree, preserve the underlying difference internally, but never mention author names, book titles, or historical reference names such as Waite or Mathers in the user-facing answer.",
    ...
    safety.isHighStakes
      ? `High-stakes categories detected: ${safety.categories.join(", ")}. Keep the answer reflective. When relevant, include these safety notices in natural language: ${safety.notices.join(" ")}`
