...
import { V1_TAROT_SOURCE_BOOK_SLUGS } from "./tarot-source-target.js";
...
export async function retrieveReadingEvidence(
  draw,
  {
    ...
    bookIds = null,
    bookSlugs = V1_TAROT_SOURCE_BOOK_SLUGS,
    matchCountPerCard = 12,
    evidenceLimitPerCard = 8,
    requireAllSources = true,
  } = {},
) {
  const expectedSourceCount = Array.isArray(bookIds) && bookIds.length
    ? bookIds.length
    : Array.isArray(bookSlugs)
      ? bookSlugs.length
      : 0;
  ...
  const cards = await Promise.all(draw.cards.map(async (card, cardIndex) => {
    const chunks = await retriever(buildReadingEvidenceQuery(draw.question, card), {
      ...
      bookIds,
      bookSlugs: bookIds ? null : bookSlugs,
      balanceBooks: expectedSourceCount > 1,
    });

    if (requireAllSources && expectedSourceCount > 1) {
      const representedBooks = new Set(chunks.map((chunk) => chunk.book_id).filter(Boolean));
      if (representedBooks.size < expectedSourceCount) {
        throw new ReadingEvidenceError(
          `「${card.nameZhTw}（${card.nameEn}）」${orientationLabel(card.orientation)}沒有完整命中 V1 的兩本來源，請檢查 ingestion 與來源篩選。`,
          "READING_SOURCE_SET_INCOMPLETE",
        );
      }
    }
