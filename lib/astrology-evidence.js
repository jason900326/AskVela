...
function evidenceItem(id, usedFor, entry, extra = {}) {
  ...
  return {
    ...
    sourceLocation: entry.sourceLocation || entry.location || "",
    secondaryLocation: entry.secondaryLocation || "",
    source: primary,
...
export function buildAstrologyEvidence({ signId, skyContext }) {
  ...
  for (const item of items) {
    const sourcePairs = [
      [item.source, item.sourceLocation],
      [item.secondarySource, item.secondaryLocation],
    ];
    for (const [book, location] of sourcePairs) {
      if (!book || !location) continue;
      const key = `${book.id}|${location}`;
  ...
  return {
    sufficient: items.length >= 7 && referencesById.size >= 2,
    items,
