...
async function generateStructured(openai, { name, schema, instructions, input, model }) {
...
export async function interpretTarotReading(
  { question, spreadId, requestId, readingId = null, selectedIndices = null },
  {
  ...
  const draw = createTarotDraw(
    { question, spreadId, idempotencyKey: requestId, selectedIndices },
    { secret: drawSecret },
  );
