...
export async function interpretTarotReading(
  ...
  // Normal readings keep the grounded Layer C synthesis as the primary result so
  // card-by-card reasoning is not compressed away. High-stakes readings retain
  // the Speech Layer as a presentation safety boundary: even if a malformed or
  // unsafe Layer C slips through a mocked/provider response, consequential advice
  // is not surfaced as the primary overview/narrative.
  const synthesis = {
    ...layerC,
