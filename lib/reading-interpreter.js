...
async function renderVelaSpeech(client, { reading, cards, layerC, safety, model }) {
  const input = buildTarotSpeechInput({ reading, cards, synthesis: layerC, safety });
  let lastSpeech = null;
  let lastError = null;
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      const speechResult = await generateStructured(client, {
        ...
        instructions: buildVelaSpeechInstructions(safety, { retry: attempt > 1 }),
        input,
        model,
        maxOutputTokens: 520,
      });
      const normalized = normalizeVelaSpeech(speechResult, layerC);
      ...
      lastError = new ReadingOutputError(
        `askvela_speech 未通過呈現規則：${normalized.violations.join(", ") || "unknown"}`,
      );
    } catch (error) {
      lastError = error;
    }
  ...
  console.warn("Vela speech renderer failed after retry; using safe fallback", lastError);

  if (safety.isHighStakes) {
    return {
      speech: buildHighStakesSpeechFallback(safety),
...
export async function interpretTarotReading(
  ...
  const renderedSpeech = await renderVelaSpeech(client, {
    reading,
    cards,
    layerC,
    safety,
