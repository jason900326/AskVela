...
function authErrorResponse(error) { return noStoreJson({ error: error.message, code: error.code }, 401); }
...
export async function GET(request) {
  try {
    ...
    const { data, error } = await supabase
      .from("dream_readings")
      .select("id, dream_text, reading_result, updated_at, expires_at")
      .gt("expires_at", now)
    ...
    return noStoreJson({ error: "目前無法載入夢境紀錄。", code: "DREAM_HISTORY_LOAD_FAILED" }, 500);
  }
}
...
export async function POST(request) {
  try {
    ...
    const { error } = await supabase.from("dream_readings").upsert({
      ...
      context_text: snapshot.contextText,
      extracted_json: snapshot.extracted,
    ...
    return noStoreJson({ error: "目前無法保存這次夢境解讀，內容仍保留在這個瀏覽器。", code: "DREAM_HISTORY_SAVE_FAILED" }, 500);
  }
}
...
export async function DELETE(request) {
  try {
    ...
    return noStoreJson({ error: "目前無法清除夢境紀錄。", code: "DREAM_HISTORY_DELETE_FAILED" }, 500);
  }
}
