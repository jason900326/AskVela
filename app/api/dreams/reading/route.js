...
export async function POST(request) {
  try {
    ...
    return noStoreJson(await createDreamReading(body));
  } catch (error) {
    if (error instanceof DreamValidationError) return noStoreJson({ error: error.message, code: error.code }, 400);
    if (error instanceof DreamSourceError) return noStoreJson({ error: error.message, code: error.code }, 503);
