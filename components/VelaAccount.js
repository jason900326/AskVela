...
function entryFingerprint(entry) {
  if (!entry) return "";
  if (entry.kind === "astrology") return JSON.stringify({ kind: "astrology", readingId: entry.readingId, result: entry.result });
  if (entry.kind === "dream") return JSON.stringify({ kind: "dream", readingId: entry.readingId, result: entry.result });
...
function historyTitle(item) {
  ...
  if (item.kind === "dream") return item.title || "一個夢";
...
export default function VelaAccount({
  ...
  async function loadHistory() {
    ...
    if (results.every((result) => !result.ok)) {
      setHistoryError(results.find((result) => result.error)?.error || "目前無法載入紀錄。");
  ...
  async function openHistoryItem(item) {
    ...
    try {
      ...
      if (item.kind === "astrology") {
        if (experience === "astrology" && onRestoreAstrology) onRestoreAstrology(data.reading);
        else {
        ...
        if (experience === "dream" && onRestoreDream) onRestoreDream(data.reading);
        else {
          window.sessionStorage.setItem(DREAM_SESSION_KEY, JSON.stringify({ version: 1, ...data.reading }));
          changeExperience("dream");
        }
      } else {
        if (experience === "tarot" && onRestoreReading) onRestoreReading(data.reading);
        else {
    ...
    <>
      ...
      {client && activeEntry && !user && (
        <aside className="saveReadingPrompt">
          <div>
            <strong>{activeEntry.kind === "astrology" ? "想把這次星座解讀留下來嗎？" : activeEntry.kind === "dream" ? "想把這次夢境解讀留下來嗎？" : "想把這次牌面留下來嗎？"}</strong>
            ...
            {!historyLoading && history.length > 0 && (
              <div className="historyList">
                {history.map((item) => {
                  ...
                  const typeLabel = item.kind === "astrology" ? `星座 · ${item.signNameZhTw}` : item.kind === "dream" ? "解夢" : `塔羅 · ${item.spreadNameZhTw}`;
                  const detail = item.kind === "astrology"
                    ? `${item.periodLabel} · ${item.localDate}`
                    : item.kind === "dream"
                      ? (item.dreamPreview || "夢境解讀")
                  ...
                  return (
                    <article className="historyItem" key={`${item.kind}-${item.readingId}`}>
                      <button className="historyOpenButton" type="button" onClick={() => openHistoryItem(item)} disabled={busyReadingId === item.readingId}>
                        <small>{formatHistoryDate(item.updatedAt)} · {typeLabel}</small>
                        <strong>{title}</strong>
