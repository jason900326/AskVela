...
- 匿名使用者先保留於瀏覽器 session；Phase 6 已獲准加入「不阻擋匿名流程」的可選帳號與跨裝置歷史。
...
以上需求統一放入 V2/V3 backlog，不得插入 V1 主線。2026-09-07 的明確範圍變更僅加入可選登入、私人跨裝置紀錄、刪除控制與 365 天保存期限；不包含社交型帳號功能，也不讓新占卜默默讀取舊紀錄。
...
- 最終牌背與牌面素材。
- 付費價格與額度。
- 是否把目前僅限單一瀏覽器 session 的匿名紀錄擴充為長期裝置內歷史。
- V2 的第二本書與多來源比較方式。
